...
export class UserRequest extends Validator {
    public async rules(): Promise<ValidationChain[]> {
        if (this.request.originalUrl == '/api/users/register') {
    ...
    private async setRegisterRules(): Promise<ValidationChain[]> {
        const chains: ValidationChain[] = [
            ...
            .custom(async (value, { req }) => {
                const user = await User.findOne({where: {username: value}, attributes: ["id"]});
                if (user) {
                    return Promise.reject(
                        i18next.t('validation.exists', { attribute: 'username' }),
                    );
                }
                return true;
            }),
